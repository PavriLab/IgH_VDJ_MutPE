...
p1 <- ggplot(DT, aes(x=stage, y=count, colour=sample, group=sample, label=substr(sample, 1, 30))) +
  ...
  geom_text_repel(data=DT[stage=="aln",], direction = "y", nudge_x = 5, segment.colour = 'black', segment.size = 0.3, segment.alpha = 0.6, size=rel(2)) +
...
p2 <- ggplot(DT, aes(x=stage, y=count, colour=sample, group=sample, label=substr(sample, 1, 30))) +
  geom_line(alpha=0.4, size=0.6) +
  geom_text_repel(data=DT[stage=="aln",], direction = "y", nudge_x = -7, nudge_y = -2, segment.colour = 'black', segment.size = 0.3, segment.alpha = 0.6, size=rel(2)) +
  scale_x_discrete(labels=flow2) +
  scale_y_log10() +
